        ...
        if len(self.Sprites.EnemyUFOs)+len(self.Sprites.EnemySHIPs)<7:
            if ((self.GameTime - self.Enemy_last_spawn) > 1):
                ...
                if(random.randint(1,15+(self.CurrentProcessor.Difficulity))<13):
                    ...
                    self.Sprites.EnemySprites.add(_EnemyUFO(self.CurrentProcessor, 805, random.randint(20, 350), 2 + (
    ...
    def PlayerDeath(self):
        ...
        for a in range(1,len(self.CurrentProcessor.ScoreBoardPoints)+1,1):
            if(self.CurrentProcessor.Player.score>int(self.CurrentProcessor.ScoreBoardPoints[a-1])):
                #print (len(self.CurrentProcessor.ScoreBoardPoints))
                _UpdateScores(self.CurrentProcessor,a-1,self.Player.score)
                break
...
class _StartScreen(object):
    ...
    def Processor(self,CurrentProcessor):
        ...
        self.Screen.window.fill((225, 225, 225))
...
def _UpdateScores(CurrentProcessor,Position,Score):
    ...
    while isTakingName==True:
        ...
        if textinput.update(events):
            ...
            CurrentProcessor.ScoreBoardPoints[Position] = Score
            CurrentProcessor.ScoreBoardNames[Position] = name
            CurrentProcessor.SaveScores()

            #print(textinput.get_text())
        # Blit its surface onto the screen
        #print (name)
        TextOnScreen(CurrentProcessor.Screen, "Congrats! You have successfully beaten a player.", 30, 170, 225)
        TextOnScreen(CurrentProcessor.Screen, "Please enter your name below. ", 30, 250, 250)
        pygame.draw.rect(CurrentProcessor.Screen.window,(225,225,225),(200,280,400,60),0)
        TextOnScreen(CurrentProcessor.Screen,"Name: ",35,225,300)
        CurrentProcessor.Screen.window.blit(textinput.get_surface(), (300, 300))
    ...
    def _Scoreboard(self):
        ...
        while isShowingScoreBoard == True:
            ...
            if keyPressed(pygame.K_o):
                ...
                self.CurrentProcessor.StartScreen.Active = True

            self.CurrentProcessor.Screen.window.fill((225, 225, 225))
            TextOnScreen(self.CurrentProcessor.Screen, "Name", 30, 50, 200)
            TextOnScreen(self.CurrentProcessor.Screen, "Score", 30, 200, 200)
            for a in range(0, 3):
                TextOnScreen(self.CurrentProcessor.Screen, str(a + 1) + '. ' + str(self.CurrentProcessor.ScoreBoardNames[a]), 30,
                             50, 220 + a * 20)
                TextOnScreen(str(self.CurrentProcessor.ScoreBoardPoints[a]), 30, 200,
                             220 + a * 20)
            pygame.display.update()
            pygame.display.flip()
