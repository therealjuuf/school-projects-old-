...
class _Enemy(pygame.sprite.Sprite):
    def __init__(self,GameWorld,x,y,velocity):
        self.groups = GameWorld.AllSprites, GameWorld.EnemySprites
        pygame.sprite.Sprite.__init__(self,self.groups)
        self.x = x
        self.y = y
...
class _GameWorld:
    ...
    def RenderSprites(self):
        ...
        for sprite in self.AllSprites:
            if sprite.x<820 and sprite.x>-20:
                self.Screen.window.blit(sprite.sprite,(sprite.x, sprite.y))
                pygame.draw.rect(self.Screen.window, (255, 0, 0), sprite.rect, 2)
            else:
                sprite.kill()
    ...
    def GenerateGround(self):
        if(len(self.GroundSprites)<2000):
            SpawnY = random.randint(300, 600)
            self.LastSpawnX-=2
            if self.LastSpawnX<0: self.LastSpawnX=1000
            CreatedRock = Rock(self, self.LastSpawnX, SpawnY)
            for rock in self.GroundSprites:
                if rock!=CreatedRock:
                    if CreatedRock.y - CreatedRock.rect[3] < rock.rect[1] + rock.rect[3] and CreatedRock.y + CreatedRock.rect[3] >rock.rect[1]:
                        if CreatedRock.x + CreatedRock.rect[3] > rock.rect[0] and CreatedRock.x - CreatedRock.rect[3] < rock.rect[0] +rock.rect[2]:
                            CreatedRock.kill()
                            pass
...
class Rock(pygame.sprite.Sprite):
    def __init__(self,GameWorld, x, y, dist=2):
        ...
        self.sprite = pygame.transform.scale(pygame.image.load("Sprites/Ground/ground.png"), (2, 400))
        self.dist = dist
