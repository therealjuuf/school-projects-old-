...
% Last Modified by GUIDE v2.5 21-Jan-2019 15:32:33
...
global appliedImage;
bright_old=0;
axesHandlesToChildObjects = findobj(handles.ori_img, 'Type', 'image');
...
switch get(handles.noisebox,'Value')
    ...
    case 2
        ...
        textLabel = sprintf('Variance');
        set(handles.var2, 'String', textLabel);
         %[x1,y1,z1]=size(currentImage2);
        % noise(1:x1,1:y1)=noise_var1;
        % local(1:x1,1:y1,1:z1)=noise_var2;
       % var= ones(1,100);
       v=im2double(currentImage);
         if (~isnan(noise_var1)&&~isnan(noise_var2))
            currentImage2=imnoise(currentImage,'localvar',v,noise_var2*rand(size(currentImage)));
         elseif (isnan(noise_var1)&&~isnan(noise_var2))
             currentImage2=imnoise(currentImage,'localvar',noise_var2*rand(size(currentImage)));
         end
    case 3
...
% --- Executes on button press in redeye.
function redeye_Callback(hObject, eventdata, handles)
% hObject    handle to redeye (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% Get the snapshot of the current frame
  data = getimage(handles.applied_img);

  
mask3 = ((data(:,:,1) > 130) & (data(:,:,2) < 60) & (data(:,:,3) < 60));
if(size(mask3,3)==3)
...
%mask3=im2bw(mask3,0.5);
mask3 = imclose(mask3, strel('disk', 32,8));
...
blue=data;
blue(mask3)=0;
%amask=cat(2,red,green,blue);


%mask3=imcomplement(mask3);
%[x1,y1]=size(diff_im);
%mask3=imresize(mask3,[x1,y1]);
%amask=ind2rgb(mask3,gray(256));
%[x1,y1]=size(data);
%data=imresize(data,[200 200]);
%amask=imresize(mask3,[200 200]);
%image=imadd(data,amask); 
%amask=im2bw(mask3,0.05);
%amask=imerode(data,amask);
%figure(2)
  imshow(mask3);
  %figure(3)
  %imshow(red);
...
% --------------------------------------------------------------------
function how_to_Callback(hObject, eventdata, handles)
% hObject    handle to how_to (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
howto;
